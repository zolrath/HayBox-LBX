...
void MultiVersusLBXFurd::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    // Bind box layout X and Y to "jump" in-game - MV detault A button
    outputs.a = inputs.x || inputs.y;

    outputs.start = !inputs.mod_y && !inputs.mod_x && inputs.start;

    // MY + Start for "Reset" in the Lab. Not supported by GameCube adapter.
    outputs.select = inputs.mod_y && !inputs.mod_x && inputs.start;

    // Home not supported by GameCube adapter.
    outputs.home = inputs.mod_x && !inputs.mod_y && inputs.start;
...
void MultiVersusLBXFurd::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
